...
{{
    config(
        ...
        cluster_by=["user", "country", "device_language"],
    )
}}
{% else %}
{{
...
select
    ...
    case when (select value.int_value from unnest(event_params) where key = 'ga_session_number') = 1 then user_pseudo_id else null end as new_users,
...
group by event_date_dt, country, device_language, user, new_users
