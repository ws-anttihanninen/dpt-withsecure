...
with
    prep as (
        ...
        where
            ...
            {% if is_incremental() %}
                ...
                {% else %}
                    -- Incrementally add new events. Filters on _TABLE_SUFFIX using
                    -- the max
    ...
    conversions as (
        select
            ...
            session_id,
            page_path,
            count(event_name) as form_submissions_cookielesspings,
            count(
                case when user_pseudo_id is not null then 1 end
            ) as form_submissions_fullconsent
...
select
    ...
    safe_divide(
        count(distinct page_views.engaged_session),
        count(distinct concat(page_views.session_id))
    ) as engagement_rate,
...
group by event_date_dt, country, page_path, user
